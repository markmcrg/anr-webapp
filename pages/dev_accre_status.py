...
from helpers import authenticate_b2, get_submissions, get_org_name_username, get_org_name_username_with_sub

def dev_accre_status():
    ...
    with top_cols[0]:
        with st.container(border=True):
            show_all_orgs = st.checkbox("Show All Organizations", key="show_all_orgs")

    if show_all_orgs:
        org_data = get_org_name_username()  # Fetch all organizations
    else:
        org_data = get_org_name_username_with_sub()  # Fetch subset with subs

    org_to_username = {item["org_name"]: item["username"] for item in org_data}

    with top_cols[1]:
        with st.container(border=True):
            selected_org_name = st.selectbox("Select an Organization", list(org_to_username.keys()))

    
    selected_username = org_to_username[selected_org_name]

    data = get_submissions(selected_username)
    st.markdown("""
                <style>
                #my-submissions {
